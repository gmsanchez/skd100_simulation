...
def generate_launch_description():
    ...
    skd100_nav2_launch_file = PathJoinSubstitution(
        [FindPackageShare("skd100_nav2_sim"), "launch", "nav2_sim.launch.py"],
    )
    
    launch_nav2_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([skd100_nav2_launch_file]),
        launch_arguments = {
            'use_sim_time': use_sim_time,
    ...
    # Delay Nav2 launch to avoid timeout and high CPU/memory load
    delayed_launch_nav2_sim = TimerAction(
        period = 10.0,       # seconds after execute the 'ros2 launch..' command
        actions = [launch_nav2_sim]
    )
    ...
    ld.add_action(delayed_launch_nav2_sim)
